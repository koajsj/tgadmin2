...
STATUS_PAGE_COUNT = 4
GROUP_PAGE_SIZE = 5
OWNER_VIEW_CACHE_TTL_SECONDS = 10.0
...
def build_owner_router(
    ...
    view_cache: dict[str, tuple[float, Any]] = {}
    ...
    async def cached(key: str, loader, *, fresh: bool = False):
        now = time.monotonic()
        if not fresh:
            cached_value = view_cache.get(key)
            if cached_value and (now - cached_value[0]) < OWNER_VIEW_CACHE_TTL_SECONDS:
                return cached_value[1]
        value = await loader()
        view_cache[key] = (now, value)
        return value

    def clear_view_cache() -> None:
        view_cache.clear()

    async def render_dashboard(*, fresh: bool = False) -> str:
        async def load() -> str:
            ...
            git = await asyncio.to_thread(inspector.git_snapshot, fresh_remote=fresh)
            ...
            return "\n".join(
                [
                    ...
                    f"最新：{_latest_label(git.current_revision, git.latest_revision)}",
                    f"CPU：{runtime.cpu_percent:.1f}%",
                    (
                        "内存："
                        f"{inspector.format_bytes(runtime.memory_used)} / "
                        f"{inspector.format_bytes(runtime.memory_total)} "
                        f"({runtime.memory_percent:.1f}%)"
                    ),
                    (
                        "磁盘："
                        f"{inspector.format_bytes(runtime.disk_used)} / "
                        f"{inspector.format_bytes(runtime.disk_total)} "
                        f"({runtime.disk_percent:.1f}%)"
                    ),
                    (
                        "数据库："
                        f"{'正常' if database.integrity_ok else '异常'} | "
                        f"{inspector.format_bytes(database.size_bytes)} | "
                        f"表 {database.table_count} 个"
                    ),
                    f"Redis：{_render_redis(redis)}",
                    f"群数量：{total_groups} | 群成员总量：{group_members} | 管理员总量：{group_admins}",
                    (
                        f"用户数量：{total_users} | 活跃用户：{active_users} | "
                        f"验证成功用户：{verified_users} | 验证失败用户：{failed_users}"
                    ),
                    (
                        f"验证：总 {verification.total} | 今日 {verification.today} | "
                        f"成功率 {verification.success_rate:.1f}% | "
                        f"失败率 {verification.failure_rate:.1f}% | "
                        f"超时率 {verification.timeout_rate:.1f}%"
                    ),
        ...
        return await cached("dashboard", load, fresh=fresh)

    async def render_status_page(page: int, *, fresh: bool = False) -> str:
        async def load() -> str:
            runtime = await asyncio.to_thread(inspector.runtime_snapshot)
            ...
            pages = [
                "\n".join(
                    [
                        ...
                        (
                            "网络："
                            f"↑ {inspector.format_bytes(runtime.net_sent)} | "
                            f"↓ {inspector.format_bytes(runtime.net_recv)}"
                        ),
        ...
        return await cached(f"status:{page}", load, fresh=fresh)

    async def render_groups_page(page: int, *, fresh: bool = False) -> tuple[str, int]:
        async def load() -> tuple[str, int]:
            total = repository.count_groups()
            total_pages = max(1, math.ceil(total / GROUP_PAGE_SIZE))
            safe_page = max(1, min(page, total_pages))
            offset = (safe_page - 1) * GROUP_PAGE_SIZE
            ...
            for index, group in enumerate(groups, start=offset + 1):
                lines.extend(
                    [
                        ...
                        (
                            f"验证：{'开启' if group.verification_enabled else '关闭'} | "
                            f"自动删除：{group.auto_delete_seconds}s"
                        ),
                        (
                            f"活跃：{_render_dt(group.last_active_at)} | "
                            f"加入：{_render_dt(group.joined_at)} | "
                            f"风险：{group.risk_level}"
                        ),
                    ]
                )
            lines.append(f"第 {safe_page}/{total_pages} 页")
            return "\n".join(lines), total_pages

        return await cached(f"groups:{page}", load, fresh=fresh)
    ...
    def pager(prefix: str, page: int, total_pages: int) -> InlineKeyboardBuilder:
        ...
        builder.button(text="刷新", callback_data=f"{prefix}:refresh:{page}")
        if page < total_pages:
            builder.button(text="下一页", callback_data=f"{prefix}:{page + 1}")
        builder.adjust(3)
        return builder

    async def send_status(message: Message, page: int = 1, *, fresh: bool = False) -> None:
        text = await render_status_page(page, fresh=fresh)
    ...
    async def send_groups(message: Message, page: int = 1, *, fresh: bool = False) -> None:
        text, total_pages = await render_groups_page(page, fresh=fresh)
        await message.answer(
            text,
            parse_mode="HTML",
    ...
    async def send_panel(message: Message, *, fresh: bool = False) -> None:
        text = await render_dashboard(fresh=fresh)
        builder = InlineKeyboardBuilder()
        builder.button(text="刷新仪表盘", callback_data="owner_panel:refresh")
    ...
    async def group_detail(message: Message, bot: Bot) -> None:
        ...
        try:
            ...
            settings_value = repository.ensure_group_settings(chat_id)
            profile = repository.touch_group_profile(
                chat_id,
                title=chat.title or str(chat_id),
                member_count=member_count,
                admin_count=len(administrators),
                verification_enabled=settings_value.enabled,
                auto_delete_seconds=settings_value.auto_delete_seconds,
    ...
    async def update(message: Message) -> None:
        ...
        if result.success and result.restarted_with != "manual" and result.restarted_with != "none":
            asyncio.create_task(
                _restart_later(audit_service, update_service, 2),
                name="owner-update-restart",
            )

    @router.callback_query(F.data == "owner_panel:refresh")
    async def callback_panel_refresh(callback: CallbackQuery) -> None:
        if not callback.message or not callback.from_user or not security_service.is_owner(callback.from_user.id):
            ...
            return
        clear_view_cache()
        text = await render_dashboard(fresh=True)
        builder = InlineKeyboardBuilder()
        builder.button(text="刷新仪表盘", callback_data="owner_panel:refresh")
        builder.button(text="/status 状态页", callback_data="owner_status:1")
        builder.button(text="/groups 群列表", callback_data="owner_groups:1")
    ...
    async def callback_status(callback: CallbackQuery) -> None:
        if not callback.message or not callback.from_user or not security_service.is_owner(callback.from_user.id):
            await callback.answer("无权限", show_alert=True)
            return
        page, fresh = _parse_callback_request(callback.data or "", "owner_status")
        if fresh:
            clear_view_cache()
        text = await render_status_page(page, fresh=fresh)
        try:
    ...
    async def callback_groups(callback: CallbackQuery) -> None:
        if not callback.message or not callback.from_user or not security_service.is_owner(callback.from_user.id):
            await callback.answer("无权限", show_alert=True)
            return
        page, fresh = _parse_callback_request(callback.data or "", "owner_groups")
        if fresh:
            clear_view_cache()
        text, total_pages = await render_groups_page(page, fresh=fresh)
        try:
...
def _parse_callback_request(data: str, prefix: str) -> tuple[int, bool]:
    payload = data.removeprefix(f"{prefix}:")
    if payload.startswith("refresh:"):
        payload = payload.removeprefix("refresh:")
        fresh = True
    else:
        fresh = False
    try:
        return max(1, int(payload)), fresh
    except ValueError:
        return 1, fresh
